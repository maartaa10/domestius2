...
export class ProtectoraService {
  ...
  getProtectoraByUsuario(usuarioId: number): Observable<Protectora> {
    console.log(
      'Llamando al endpoint /api/protectora/usuario con ID:',
      usuarioId
    );
    return this.http
      .get<Protectora>(
        `https://apidomestius-production.up.railway.app/api/protectora/usuario/${usuarioId}`
      )
      .pipe(
        tap((response) =>
          console.log(
            'Respuesta del endpoint /api/protectora/usuario:',
            response
          )
        ),
        catchError((error) => {
          ...
          return throwError(
            () => new Error('No se pudo obtener la protectora.')
          );
        })
      );
  }
  createProtectora(protectoraData: FormData): Observable<Protectora> {
    return this.http.post<Protectora>(
      `${this.apiUrl}/protectora/create`,
      protectoraData
    );
  }
}
